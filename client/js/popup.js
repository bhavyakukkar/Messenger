...
var lastNewContactUsername = "";
var lastNewContactMessage = "";

var hostUrl = "http://bhavyakukkar.000webhostapp.com/messenger/";
...
function sendMessage(senderId, receiverId, message, sendKey) {
...
function loginAttempt(loginUsername, loginPassword) {
    ...
    var url = hostUrl+"server/php/login_user.php"+urlParameters;
...
function registerAttempt(registerUsername, registerPassword) {
    ...
    var url = hostUrl+"server/php/create_user.php"+urlParameters;
...
function retrieveContacts() {
    ...
    var url = hostUrl+"server/php/fetch_contacts.php"+urlParameters;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
...
function updateContacts(retrievedContacts) {
    ...
    contacts.insertBefore(retrievedContacts, document.getElementById("new-contact-button"));

    addContactButtons(retrievedContacts);
}


//Add Event Listener to New-Contact Button
function addNewContactButton() {
    document.getElementById("new-contact-button").addEventListener('click', function() {
        hideScreen("main");
        showScreen("new-contact");

        document.getElementById("search-username-error").style.display = "none";

        //New Contact Search Attempt
        document.getElementById("search-button").addEventListener('click', function() {
            
            var searchUsername = document.getElementById("search-username").value;
            var searchMessage = document.getElementById("search-message").value;

            document.getElementById("search-username-error").style.display = "none";

            if(searchUsername && searchMessage)
                searchAttempt(searchUsername, searchMessage);
        });
    })
}
function searchAttempt(searchUsername, searchMessage) {
    ...
    var url = hostUrl+"server/php/user_exist.php"+urlParameters;
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            var parser = new DOMParser();
            var responseDom = parser.parseFromString(this.responseText, "text/html");
            var requestedDom = responseDom.getElementById("user-exist");
            
            switch(requestedDom.innerText) {
                case "1":
                    hideScreen("new-contact");
                    sendMessage(username, lastNewContactUsername, lastNewContactMessage, key);
                    showScreen("main");
                    main();
                    break;
                
                case "3":
                    document.getElementById("search-username-error").style.display = "block";
                    break;
...
function retrieveMessages() {
    ...
    var url = hostUrl+"server/php/fetch_messages.php"+urlParameters;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
